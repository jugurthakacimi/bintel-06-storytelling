...
STORYTELLING_CHART_FILE_3: Final[Path] = (
    CHARTS_OUTPUT / "storytelling_regional_share_jugurtha.png"
)
STORYTELLING_CHART_FILE_4: Final[Path] = (
    CHARTS_OUTPUT / "storytelling_category_month_heatmap_jugurtha.png"
)
...
# === Section 2.5 DEFINE A SUMMARIZE REGIONAL SALES FUNCTION ===


def summarize_regional_sales(df_reporting: pd.DataFrame) -> pd.DataFrame:
    """Summarize total sales by region, across the whole dataset.

    WHY: Before drilling into East, it helps the reader see how big
    a slice East actually is compared to the other regions. This
    gives the audience context for why the deep-dive matters.

    Args:
        df_reporting: Complete reporting-ready sales data.

    Returns:
        DataFrame with Region and TotalSales columns, sorted descending.
    """
    LOG.info("Summarizing total sales by region")

    df_regional_sales: pd.DataFrame = (
        df_reporting.groupby("Region", as_index=False)
        .agg(TotalSales=("SaleAmount", "sum"))
        .sort_values("TotalSales", ascending=False)
    )

    df_regional_sales["TotalSales"] = df_regional_sales["TotalSales"].round(2)

    LOG.info(f"  Regions summarized: {df_regional_sales.shape[0]}")
    return df_regional_sales


# === Section 2.6 DEFINE A SUMMARIZE CATEGORY BY MONTH SALES FUNCTION ===


def summarize_category_month_sales(
    df_reporting: pd.DataFrame,
    selected_region: str,
) -> pd.DataFrame:
    """Build a Category x Month pivot table of sales for one region.

    WHY: The heatmap shows every category's seasonality at once,
    not just the single leading category from the line chart. This
    can reveal secondary patterns, such as a category that is small
    overall but spikes hard in one month.
    ...
    Returns:
        Pivoted DataFrame indexed by Category, columns are YearMonth,
        values are total sales (0.0 where no sales occurred).
    """
    LOG.info(f"Summarizing category x month sales for Region = {selected_region!r}")
    ...
    # Sort months chronologically left-to-right.
    df_pivot = df_pivot.reindex(sorted(df_pivot.columns), axis=1)

    LOG.info(f"  Heatmap shape (categories x months): {df_pivot.shape}")
    return df_pivot


# === Section 2.7 DEFINE AN IDENTIFY KEY RESULTS FUNCTION ===
...
def main() -> None:
    ...
    # STEP 4b: GATHER THE DATA FOR THE THIRD AND FOURTH CHARTS.
    LOG.info("CALL a function to summarize regional sales share........")
    df_regional_sales = summarize_regional_sales(df_reporting)

    LOG.info("CALL a function to summarize category x month sales........")
    df_category_month_sales = summarize_category_month_sales(
        df_reporting,
        SELECTED_REGION,
    )
    ...
    LOG.info("CALL matplotlib directly to plot regional sales share (pie)........")
    fig3, ax3 = plt.subplots(figsize=(7, 7))
    ax3.pie(
        df_regional_sales["TotalSales"],
    ...
    ax3.axis("equal")
    plt.savefig(STORYTELLING_CHART_FILE_3, bbox_inches="tight")
    plt.close(fig3)
    log_path(LOG, "Saved regional share chart:", STORYTELLING_CHART_FILE_3)

    LOG.info("CALL seaborn directly to plot category x month heatmap........")
    fig4, ax4 = plt.subplots(figsize=(10, 6))
    sns.heatmap(
        df_category_month_sales,
        annot=True,
        fmt=",.0f",
        cmap="Blues",
        linewidths=0.5,
    ...
    ax4.set_title(f"Category x Month Sales Heatmap in {SELECTED_REGION}")
    ax4.set_xlabel("Month")
    ax4.set_ylabel("Category")
    plt.savefig(STORYTELLING_CHART_FILE_4, bbox_inches="tight")
    plt.close(fig4)
    log_path(LOG, "Saved category/month heatmap:", STORYTELLING_CHART_FILE_4)
